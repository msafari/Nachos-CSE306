...
public class Options {
    ...
    private static final String copyright = "Copyright (c) 1992-1993 The Regents of the University of California.\n"
    ...
    /**
     * If non-zero, then a seed to use for the simulation random number
     * generator. If zero, then the simulation will be randomized.
     */
    public int RANDOM_SEED = 0;

    /**
     * If non-zero, the maximum number of ticks that the simulation will run
     * for. If the current simulated time reaches this value, the simulation
     * will terminate abruptly. Not that if the simulated time reaches
     * Integer.MAX_VALUE, things might not work properly, so it probably doesn't
     * make sense to run any longer than Integer.MAX_VALUE-1.
     */
    public int TIME_LIMIT_TICKS = Integer.MAX_VALUE - 1;

    /**
     * Target simulation rate. If nonzero, then the simulation will throttle to
     * no more than roughly this many ticks per second of real time. If zero,
     * then there is no throttle.
     */
    public int TICKS_PER_SECOND = 1000000;

    /**
     * Single-stepping user programs. If true, then user program execution is
     * controlled by a crude debugger, which allows the program to be
     * single-stepped. This might not be very useful with more than one CPU.
     */
    ...
    /**
     * The number of CPUs on the system. If there is just one CPU, then there is
     * no real concurrent execution of threads and the spinlock synchronization
     * facility is bypassed. If there is more than one CPU, then threads running
     * on different CPUs will run concurrently as Java threads, and will run
     * truly concurrently if the host system is a multi-CPU system and the JVM
     * supports it.
     */
     ...
     * 
     * NOTE: Configuring a console will start the console driver, which enables
     * keyboard interrupts. Once keyboard interrupts have been enabled, the
     * simulation will run forever unless explicitly stopped.
     */
	...
	processOptions(new Spec[] {
		...
		new Spec(
			"-tl", // set simulation time limit
			new Class[] { Integer.class }, "Usage: -tl <ticks>",
			new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec(
			"-sr", // set simulation rate
			new Class[] { Integer.class },
			"Usage: -sr <ticks per second>", new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec(
			"-rs", // set simulation random number generator seed
			new Class[] { Integer.class }, "Usage: -rs <seed>",
			new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec("-ps", // enable pre-emptive scheduling using per-CPU
				// scheduling timers
			new Class[] {}, null, new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec(
			"-ncpu", // set the number of CPUs to use
			new Class[] { Integer.class }, "Usage: -ncpu <ncpus>",
			new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec(
			"-ncon", // set the number of consoles to use
			new Class[] { Integer.class },
			"Usage: -ncon <nconsoles>", new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec(
			"-df", // set the base filename for simulated disks
			new Class[] { String.class }, "Usage: -df <filename>",
			new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
		...
		new Spec("-CB", // enable CyclicBarrier test
			new Class[] {}, null, new Options.Action() {
			    public void processOption(String flag,
				    Object[] params) {
    ...
    /**
     * Class whose objects represent the specification of a particular type of
     * option to look for in the argument list.
     */
	 ...
	 * 
	 * @param flag
	 *            The flag that signals the start of the option. This can be
	 *            any string, but conventionally option flags start with
	 *            "-".
	 * @param types
	 *            Array of Class objects that specifies the number and types
	 *            of the parameters that should follow this option flag.
	 * @param usage
	 *            Message to be printed when the proper number or types of
	 *            parameters are not found for an option, or null for no
	 *            useful message.
	 * @param action
	 *            Action to be taken when an occurrence of this option and
	 *            its associated parameters is encountered in the argument
	 *            list.
	 */
	 ...
	 * @param args
	 *            Iterator that provides the remaining arguments. If the
	 *            option flag matches this spec, then any parameters
	 *            required will be obtained from this iterator.
	 */
	public void match(String flag, Iterator<String> args) {
		...
		if (!args.hasNext()) {
		    System.out.println((usage != null ? usage + "\n" : "")
    ...
    /**
     * Interface implemented by objects to be invoked when an option has been
     * encountered.
     */
	 ...
	 * 
	 * @param flag
	 *            The option flag that was matched.
	 * @param params
	 *            The parameters to the option.
	 */
	public void processOption(String flag, Object[] params);
    }

    /**
     * Method to scan the argument list looking for options matching a list of
     * specifications, and dispatching option processing when an option
